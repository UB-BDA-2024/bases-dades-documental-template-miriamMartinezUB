...
def test_create_sensor_velocitat():
    response = client.post("/sensors",
                           json={"name": "Sensor Velocitat 1", "latitude": 1.0, "longitude": 1.0, "type": "Velocitat",
...
def test_post_sensor_2_data():
    response = client.post("/sensors/2/data",
                           json={"velocity": 45.0, "battery_level": 1.0, "last_seen": "2020-01-01T00:00:00.000Z"})
    assert response.status_code == 200
...
def test_post_sensor_data_not_exists():
    response = client.post("/sensors/3/data", json={"temperature": 1.0, "humidity": 1.0, "battery_level": 1.0,
                                                    "last_seen": "2020-01-01T00:00:00.000Z"})
...
def test_update_sensor_1_data():
    response = client.post("/sensors/1/data", json={"temperature": 2.0, "humidity": 2.0, "battery_level": 1.9,
...
def test_update_sensor_2_data():
    response = client.post("/sensors/2/data",
                           json={"velocity": 46.0, "battery_level": 1.9, "last_seen": "2020-01-01T00:00:01.000Z"})
    assert response.status_code == 200
